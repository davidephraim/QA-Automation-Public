...
def process_checksum(company_format):
    ...
    static_checksum = sha256_checksum(
        stat["file"]["pdf_1"]
    )
...
def create_employee_timesheet(
    ...
    log_step(
        f"FLOW | {company_format} | {timesheet_config}"
    )
...
def open_admin_wait(playwright, env):
