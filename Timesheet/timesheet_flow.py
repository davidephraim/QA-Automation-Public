...
def run_checksum(page, fmt, ts_type):
    ...
    if result:
        log_step(page, f"CHECKSUM RESULT: PASS ({fmt}-{ts_type})")
    else:
        log_step(page, f"CHECKSUM RESULT: FAILED ({fmt}-{ts_type})")
...
def test_timesheet_flow(playwright: Playwright, env_config, username, password, company_format):
    ...
    for fmt, format_data in timesheet_formats.items():
        ...
        if not format_id:
            logging.info(f"Skip {fmt} (no format id)")
            continue

            logging.info(f"========== START FORMAT: {fmt.upper()} ==========")
        ...
        for ts_type in timesheet_types:
            logging.info(f"[DS ON] {fmt.upper()} | {ts_type.upper()}")
        ...
        for ts_type in timesheet_types:
            logging.info(f"[DS OFF] {fmt.upper()} | {ts_type.upper()}")
            ...
            while True:
                try:
                    ...
                    # CHECKSUM
                    run_checksum(hr_page, fmt, ts_type)
        ...
        logging.INFO(f"========== END FORMAT: {fmt.upper()} ==========\n")
